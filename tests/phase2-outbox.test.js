...
import { BullMqOutboxPublisher, dispatchPendingOutboxEvents, publishOutboxEvent, } from "../app/services/outbox/dispatcher.server";
...
import { createQueueRegistry, } from "../app/queues/queues.server";
...
describe("Phase 2 queue job descriptors", () => {
    it("routes diagnostic events to the maintenance queue with a BullMQ-safe deterministic job ID", () => {
        ...
        const descriptor = describeOutboxJob(event, new Date("2026-09-10T00:00:00.000Z"));
    ...
    it("uses the durable outbox event as the order-create delivery dedupe key", () => {
        const descriptor = describeOutboxJob(createOutboxEventFixture({
...
describe("processMaintenanceJob", () => {
    it("handles duplicate diagnostic deliveries idempotently", async () => {
        ...
        await expect(processMaintenanceJob(job, { prisma, logger: testLogger })).resolves.toMatchObject({
            status: "handled",
            eventId: event.id,
            shopId: event.shopId,
...
const describeIfInfrastructure = databaseUrl && redisUrl ? describe : describe.skip;
describeIfInfrastructure("Phase 2 outbox and BullMQ integration", () => {
    ...
    it("publishes the same event more than once without creating duplicate BullMQ jobs", async () => {
        ...
        const jobCounts = await registry.maintenance.getJobCounts("waiting", "prioritized", "delayed");
        ...
        expect(jobCounts.waiting + jobCounts.prioritized + jobCounts.delayed).toBe(1);
    });
    it("lets a worker started after dispatch handle the diagnostic job", async () => {
        ...
        try {
            await expect(queuedJob?.waitUntilFinished(events, 5_000)).resolves.toMatchObject({
                status: "handled",
                eventId: diagnostic.event.id,
    ...
    function createTestQueueRegistry(name) {
    ...
    function createTestQueueEvents(name) {
...
async function createDiagnostic(prisma) {
...
function createOutboxEventFixture(overrides = {}) {
