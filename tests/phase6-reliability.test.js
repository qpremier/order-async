...
import { nextImportPollDelay, shouldPollImport, } from "../app/services/imports/import-polling";
import { buildImportStatusEtag, createImportStatusLoader, etagMatches, } from "../app/services/imports/import-status.server";
import { replayDeadLetterOrder, requestAmbiguousReconciliation, } from "../app/services/orders/order-attention.server";
...
import { ingestAppLifecycleWebhook, processAppLifecycleWebhook, } from "../app/services/webhooks/app-lifecycle.server";
...
describe("Phase 6 local status polling", () => {
    it("returns local status with an ETag and never calls Shopify GraphQL", async () => {
        ...
        const first = await loader({
            request: new Request("https://example.com/app/api/imports/batch-1/status"),
        ...
        const unchanged = await loader({
            request: new Request("https://example.com/app/api/imports/batch-1/status", { headers: { "If-None-Match": etag } }),
            params: { batchId: "batch-1" },
        });
...
describeIfDatabase("Phase 6 dead letter and webhook safety", () => {
    ...
    it("persists permanent failures and replays the same order identity", async () => {
        ...
        expect(describeOutboxJob(event).jobId).toBe(`dead-letter-replay__${event.id}`);
        await expect(replayDeadLetterOrder(prisma, {
            shopId: fixture.shopId,
            orderIntentId: fixture.orderIntentId,
            replayedBy: "merchant",
    ...
    it("deduplicates scope webhooks and pauses queued order work", async () => {
        ...
        expect(await prisma.webhookReceipt.count({
        ...
        await expect(processOrderJob(orderJob(fixture), {
            prisma,
            rateGate: {},
...
async function createOrderFixture(prisma, status) {
    ...
    const intent = await prisma.orderIntent.create({
        data: {
            ...
            shopifyOrderGid: status === "SUCCEEDED" ? "gid://shopify/Order/123" : null,
