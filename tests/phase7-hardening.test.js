...
import { createDraftImport, ExternalOrderConflictError, } from "../app/services/imports/import-domain.server";
...
import { createLogger, createSilentLogger, getRequestCorrelationId, } from "../app/services/logging/logger.server";
...
import { createAsyncFailureInjector, injectedFailure, injectedResult, } from "./helpers/failure-injection";
describe("Phase 7 observability and safe transport", () => {
    ...
    it("accepts safe inbound correlation IDs and replaces unsafe ones", () => {
        expect(getRequestCorrelationId(new Request("https://app.example.com", {
...
describe("Phase 7 sample imports and failure injection", () => {
    it("keeps the scripted failure sequence deterministic", async () => {
        const injector = createAsyncFailureInjector([
            injectedFailure(new Error("injected worker crash")),
            injectedResult("recovered"),
        ]);
        await expect(injector.invoke("first")).rejects.toThrow("injected worker crash");
    ...
    it("parses the valid, missing-SKU, and duplicate-identity samples", async () => {
        ...
        const duplicateIdentity = await parseExample("orders-duplicate-external-id.csv");
        expect(valid).toHaveLength(2);
        expect(valid.find((order) => order.externalOrderId === "ERP-1001")?.lines).toHaveLength(2);
...
describeIfDatabase("Phase 7 representative import pipeline", () => {
    ...
    it("runs CSV validation through durable order completion with mocked Shopify", async () => {
        ...
        const options = processorOptions(prisma, (query, requestOptions) => graphql.invoke(query, requestOptions));
        expect(events).toHaveLength(2);
        for (const event of events) {
            const descriptor = describeOutboxJob(event);
            expect(JSON.stringify(descriptor.data)).not.toContain("@example.com");
            await expect(processOrderJob({
                id: descriptor.jobId,
                name: descriptor.jobName,
                data: descriptor.data,
                attemptsMade: 0,
            }, options)).resolves.toMatchObject({ status: "succeeded" });
        }
        expect(graphql.calls).toHaveLength(2);
        await expect(getImportStatus(prisma, {
            shopId: shop.id,
            batchId: created.batch.id,
        ...
        const conflictingOrders = await parseExample("orders-duplicate-external-id.csv");
...
async function parseExample(name) {
    const contents = await readFile(new URL(`../examples/${name}`, import.meta.url), "utf8");
    return parseImportCsv(new File([contents], name, { type: "text/csv" }), {
        maxBytes: 100_000,
        maxRows: 100,
    });
}
function catalogVariant(shopId, sku, numericId) {
    return {
        shopId,
...
function graphqlResponse(orderGid, orderName) {
    return {
        json: async () => ({
            data: {
                orderCreate: {
