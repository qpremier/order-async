...
import { runFullCatalogSync, } from "../app/services/catalog/catalog-sync.server";
...
describeIfDatabase("Phase 3 catalog synchronization", () => {
    ...
    it("syncs a full catalog across multiple Shopify cursor pages", async () => {
        ...
        expect(admin.graphql).toHaveBeenNthCalledWith(1, expect.any(String), expect.objectContaining({
            variables: expect.objectContaining({ after: null, first: 1 }),
        }));
        expect(admin.graphql).toHaveBeenNthCalledWith(2, expect.any(String), expect.objectContaining({
    ...
    it("resumes an interrupted catalog sync from the saved checkpoint", async () => {
        ...
        expect(admin.graphql).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
    ...
    it("preserves the previous cache when Shopify sync fails", async () => {
        ...
        await expect(runFullCatalogSync({
            prisma,
            shopId: shop.id,
            syncRunId: syncRun.id,
            admin,
...
async function createShop(prisma, overrides = {}) {
...
function createThrowingAdmin(error) {
    return {
        graphql: vi.fn(async () => {
            throw error;
        }),
