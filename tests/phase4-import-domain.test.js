...
import { autoResolveDraftImportSkus, createDraftImport, ExternalOrderConflictError, getImportDetails, listMappingCandidates, } from "../app/services/imports/import-domain.server";
...
describeIfDatabase("Phase 4 import domain", () => {
    ...
    it("creates a DRAFT batch and blocks only orders with missing mappings", async () => {
        ...
        expect(intents.map((intent) => [intent.externalOrderId, intent.status])).toEqual([
    ...
    it("auto-matches the spreadsheet text marker on a numeric SKU", async () => {
        ...
        const result = await createDraftImport(prisma, request(domain, await parseOrders([row("order-1", "'1901743")])));
    ...
    it("rechecks unresolved drafts and leaves genuinely different SKUs for manual mapping", async () => {
        const domain = shopDomain();
        const shop = await prisma.shop.create({ data: { domain } });
        const result = await createDraftImport(prisma, request(domain, await parseOrders([
            row("matching-order", "'1901743"),
            row("different-order", "1901743-DIFFERENT"),
        ])));
        const variant = await createVariant(prisma, shop.id, "1901743", "numeric");
        await expect(autoResolveDraftImportSkus(prisma, {
            shopId: shop.id,
        ...
        expect(await prisma.importBatch.findUniqueOrThrow({
    ...
    it("requires explicit mapping for duplicate catalog SKUs and then unblocks the order", async () => {
        ...
        const result = await createDraftImport(prisma, request(domain, await parseOrders([row("order-1", "dup")])));
        expect(result.batch.needsAttentionOrders).toBe(1);
        expect(await prisma.orderIntent.findFirstOrThrow({ where: { shopId: shop.id } })).toMatchObject({ status: "AMBIGUOUS_MAPPING" });
        ...
        expect(await prisma.orderIntent.findFirstOrThrow({ where: { shopId: shop.id } })).toMatchObject({ status: "READY" });
        expect(await prisma.importBatch.findUniqueOrThrow({
            where: { id: result.batch.id },
        })).toMatchObject({
            readyOrders: 1,
    ...
    it("paginates order intents with opaque keyset cursors", async () => {
        ...
        const result = await createDraftImport(prisma, request(domain, await parseOrders([row("order-1", "SKU-1"), row("order-2", "SKU-1")])));
        ...
        expect(secondPage?.intentsPage.items[0].id).not.toBe(firstPage?.intentsPage.items[0].id);
    });
    ...
    it("returns the original batch for repeated and concurrent idempotency keys", async () => {
        ...
        expect(new Set([first.batch.id, second.batch.id, third.batch.id]).size).toBe(1);
        expect(await prisma.importBatch.count({ where: { shopId: shop.id } })).toBe(1);
    });
    it("reuses the same external order and hash in a later batch", async () => {
        ...
        expect(await prisma.orderIntent.count({ where: { shopId: shop.id } })).toBe(1);
        expect(await prisma.importBatchOrderIntent.count({
    ...
    it("raises a conflict instead of overwriting a changed external order", async () => {
        ...
        const shop = await prisma.shop.create({ data: { domain } });
        await createVariant(prisma, shop.id, "SKU-1", "one");
        await createDraftImport(prisma, request(domain, await parseOrders([row("order-1", "SKU-1", "10.00")])));
        await expect(createDraftImport(prisma, {
            ...request(domain, await parseOrders([row("order-1", "SKU-1", "11.00")])),
            idempotencyKey: crypto.randomUUID(),
    ...
    it("prevents cross-shop details and mapping access", async () => {
        ...
        const variant = await createVariant(prisma, attacker.id, "SKU-X", "attacker");
        const result = await createDraftImport(prisma, request(victimDomain, await parseOrders([row("order-1", "MISSING")])));
        expect(await getImportDetails(prisma, {
            shopId: attacker.id,
            batchId: result.batch.id,
            first: 20,
        })).toBeNull();
        await expect(applySkuMapping(prisma, {
            shopId: attacker.id,
            batchId: result.batch.id,
...
const headers = "external_order_id,processed_at,email,currency,sku,quantity,unit_price";
function request(shopDomainValue, orders) {
    return {
...
async function parseOrders(rows) {
    return parseImportCsv(new File([[headers, ...rows].join("\n")], "orders.csv"), {
        maxBytes: 100_000,
        maxRows: 100,
    });
}
function row(externalOrderId, sku, price = "10.00") {
...
function createVariant(prisma, shopId, sku, suffix) {
