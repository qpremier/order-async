...
import { ImportLimitError, ImportValidationError, parseImportCsv, } from "../app/services/imports/import-parser.server";
describe("Phase 4 streaming CSV parser", () => {
    it("groups rows and produces a stable canonical payload hash", async () => {
        const first = await parseImportCsv(csvFile(`${headers}\n${row("order-1", "SKU-B", "2", "10")}\n${row("order-1", "sku-a", "1", "4.5")}`), limits);
        const reordered = await parseImportCsv(csvFile(`${headers}\n${row("order-1", "sku-a", "1", "4.50")}\n${row("order-1", "SKU-B", "2", "10.00")}`), limits);
    ...
    it("returns safe row and field errors for invalid input", async () => {
        await expect(parseImportCsv(csvFile(`${headers}\n${row("order-1", "SKU-A", "0", "12.345")}`), limits)).rejects.toMatchObject({
    ...
    it("rejects missing headers and inconsistent order-level values", async () => {
        await expect(parseImportCsv(csvFile("external_order_id,sku\norder-1,SKU-A"), limits)).rejects.toBeInstanceOf(ImportValidationError);
        await expect(parseImportCsv(csvFile(`${headers}\n${row("order-1", "SKU-A", "1", "10", "first@example.com")}\n${row("order-1", "SKU-B", "1", "10", "second@example.com")}`), limits)).rejects.toMatchObject({
            issues: [expect.objectContaining({ row: 3 })],
        });
    });
    it("enforces row and byte limits", async () => {
        await expect(parseImportCsv(csvFile(`${headers}\n${row("order-1", "SKU-A", "1", "10")}\n${row("order-2", "SKU-B", "1", "10")}`), { maxBytes: 10_000, maxRows: 1 })).rejects.toBeInstanceOf(ImportLimitError);
        await expect(parseImportCsv(csvFile(`${headers}\n${row("order-1", "SKU-A", "1", "10")}`), {
            maxBytes: 10,
            maxRows: 100,
...
function row(externalOrderId, sku, quantity, unitPrice, email = "buyer@example.com") {
