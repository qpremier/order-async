...
import { describeOutboxJob, JOB_NAMES, OUTBOX_EVENT_TYPES, } from "../app/queues/jobs";
...
import { buildOrderSourceIdentifier, confirmImportBatch, markOrderIntentSucceeded, } from "../app/services/orders/order-state.server";
import { ShopifyRateGate } from "../app/services/shopify/shopify-rate-gate.server";
import { OrderWorkDeferredError, processOrderJob, } from "../worker/processors/order.processor";
...
describeIfDatabase("Phase 5 order pipeline", () => {
    ...
    it("confirms ready intents and creates outbox work without calling Shopify", async () => {
        ...
        expect(await prisma.outboxEvent.count({ where: { shopId: fixture.shopId } })).toBe(1);
    });
    it("lets only one of two workers claim and create an order", async () => {
        ...
        expect(results.some((result) => (result.status === "rejected" &&
            ...
            (result.status === "fulfilled" &&
                result.value.status === "not-claimed"))).toBe(true);
        expect(await prisma.orderIntent.findUniqueOrThrow({
            where: { id: fixture.orderIntentId },
        })).toMatchObject({
            status: "SUCCEEDED",
    ...
    it("keeps batch counters consistent when many orders finish concurrently", async () => {
        ...
        await Promise.all(fixture.orderIntentIds.map((orderIntentId, index) => markOrderIntentSucceeded(prisma, {
            shopId: fixture.shopId,
            orderIntentId,
        ...
        await expect(prisma.importBatch.findUniqueOrThrow({
    ...
    it("reconciles a stale processing claim after a worker restart instead of blindly creating", async () => {
        ...
        const graphql = vi.fn();
        await expect(processOrderJob(job(JOB_NAMES.orderCreate, fixture), processorOptions(prisma, graphql))).resolves.toMatchObject({ status: "ambiguous" });
    ...
    it("stores Shopify user errors as a permanent safe failure", async () => {
        ...
        await expect(processOrderJob(job(JOB_NAMES.orderCreate, fixture), processorOptions(prisma, graphql))).resolves.toMatchObject({ status: "permanent-failure" });
        expect(await prisma.orderIntent.findUniqueOrThrow({
            where: { id: fixture.orderIntentId },
        })).toMatchObject({
            status: "DEAD_LETTER",
    ...
    it("retries the development-store order resource limit without dead-lettering", async () => {
        ...
        await expect(processOrderJob(job(JOB_NAMES.orderCreate, fixture), processorOptions(prisma, graphql))).rejects.toMatchObject({
        ...
        expect(await prisma.deadLetterRecord.count({
    ...
    it("delays Shopify throttling without dead-lettering", async () => {
        ...
        await expect(processOrderJob(job(JOB_NAMES.orderCreate, fixture), processorOptions(prisma, graphql))).rejects.toBeInstanceOf(OrderWorkDeferredError);
        expect(await prisma.orderIntent.findUniqueOrThrow({
            where: { id: fixture.orderIntentId },
        })).toMatchObject({
            status: "RETRY_WAIT",
    ...
    it("retries safely when the Admin client fails before request dispatch", async () => {
        ...
        await expect(processOrderJob(job(JOB_NAMES.orderCreate, fixture), options)).rejects.toBeInstanceOf(OrderWorkDeferredError);
        expect(await prisma.orderIntent.findUniqueOrThrow({
            where: { id: fixture.orderIntentId },
        })).toMatchObject({
            status: "RETRY_WAIT",
    ...
    it("reconciles exactly one matching order without another create", async () => {
        ...
        await expect(processOrderJob(job(JOB_NAMES.orderReconcileAmbiguous, fixture), processorOptions(prisma, graphql))).resolves.toMatchObject({ status: "reconciled" });
        expect(graphql).toHaveBeenCalledWith(expect.stringContaining("OrderRelayReconcileOrder"), expect.objectContaining({
            variables: {
    ...
    ])("keeps unresolved reconciliation visible for merchant review", async (nodes, status) => {
        const fixture = await ambiguousImport(prisma);
        const graphql = vi
            .fn()
            .mockResolvedValue(response({ data: { orders: { nodes } } }));
        await expect(processOrderJob(job(JOB_NAMES.orderReconcileAmbiguous, fixture), {
...
describeIfRedis("Phase 5 Shopify Redis rate gate", () => {
    ...
    const prefixes = [];
    beforeAll(async () => {
        const { createBullMqRedisConnection } = await import("../app/queues/connection.server");
    ...
    it("restores budget, honors observations, and reserves background headroom", async () => {
        ...
        expect((await gate.reserve("shop-b", 20, "background")).allowed).toBe(false);
    });
    it("enforces a shared five-order rolling window after Shopify reports the resource limit", async () => {
        ...
        expect(await gate.reserveOrderCreate("shop-a", "before-activation")).toMatchObject({ allowed: true });
        expect(await gate.activateOrderCreateLimit("shop-a")).toBe(61_000);
        expect(await gate.reserveOrderCreate("shop-a", "during-cooldown")).toMatchObject({ allowed: false, retryAfterMs: 61_050 });
        expect(await gate.reserveOrderCreate("shop-b", "other-shop")).toMatchObject({ allowed: true });
        now += 61_000;
        const reservations = await Promise.all(Array.from({ length: 6 }, (_, index) => gate.reserveOrderCreate("shop-a", `next-window-${index}`)));
        ...
        expect(await gate.reserveOrderCreate("shop-a", "next-window-0")).toMatchObject({ allowed: true });
        now += 61_000;
        expect(await gate.reserveOrderCreate("shop-a", "following-window")).toMatchObject({ allowed: true });
    });
...
function processorOptions(prisma, graphql) {
    const evalMock = vi.fn(async (script) => {
        if (script.includes("order-create-resource-reserve"))
            return [1, 0];
        if (script.includes("order-create-resource-activate"))
            return 61_000;
...
async function queuedImport(prisma) {
    const fixture = await createReadyImport(prisma);
    await confirmImportBatch(prisma, {
        shopId: fixture.shopId,
        batchId: fixture.batchId,
...
async function ambiguousImport(prisma) {
    const fixture = await queuedImport(prisma);
    await prisma.orderIntent.update({
        where: { id: fixture.orderIntentId },
        data: {
...
async function createReadyImport(prisma, orderCount = 1) {
    ...
    const orders = await parseImportCsv(new File([
        [
            "external_order_id,processed_at,email,currency,sku,quantity,unit_price",
            ...rows,
        ].join("\n"),
    ], "orders.csv"), { maxBytes: 100_000, maxRows: 100 });
