...
import { createSilentLogger, sanitizeErrorMessage, } from "../app/services/logging/logger.server.js";
...
import { OrderWorkDeferredError, processOrderJob, } from "./processors/order.processor.js";
import { ShopifyRateLimitDeferredError, } from "../app/services/shopify/shopify-rate-gate.server.js";
export function createQueueWorkers(options) {
    ...
    const workers = [
        createWorker({
            ...
            processor: (job) => {
                ...
                return processOrderJob(job, {
                    ...
                    orderReconcileEstimatedCost: options.orderReconcileEstimatedCost ?? 10,
        ...
        createWorker({
            ...
            prefix: options.prefix,
            logger,
            processor: (job) => processCatalogJob(job, {
                prisma: options.prisma,
        ...
        createWorker({
            ...
            prefix: options.prefix,
            logger,
            processor: (job) => processMaintenanceJob(job, {
                prisma: options.prisma,
                logger,
            }),
        }),
...
function createWorker(options) {
    const worker = new Worker(options.queueName, async (job, token) => {
        ...
        try {
            ...
            if (error instanceof OrderWorkDeferredError ||
