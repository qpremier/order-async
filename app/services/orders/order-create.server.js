...
export async function createShopifyOrder(admin, intent) {
    ...
    try {
        ...
        return {
            ...
            message: "Shopify did not return a conclusive order creation response; reconciliation is required.",
        };
    }
    ...
    if (!parsed.success) {
        return {
            ...
            message: "Shopify returned an unexpected order creation response; reconciliation is required.",
        };
    }
    ...
    const resourceThrottle = payload?.userErrors.find((error) => isOrderCreateResourceThrottle(error.message));
    if (resourceThrottle) {
        return {
            outcome: "retry",
            category: "THROTTLED",
    ...
    if (!payload?.order) {
        return {
            ...
            message: "Shopify did not confirm the created order; reconciliation is required.",
        };
    }
    return {
        outcome: "succeeded",
...
export function isOrderCreateResourceThrottle(message) {
    return /too many attempts[.!]?\s*please try again later[.!]?/i.test(message.trim());
}
export function buildOrderCreateInput(intent) {
...
function classifyTopLevelErrors(errors) {
    if (!errors?.length)
        return null;
    const error = errors[0];
    const code = error.extensions?.code ?? "GRAPHQL_ERROR";
    const message = sanitizeShopifyMessage(error.message ?? "Shopify rejected the order request.");
    if (code === "THROTTLED") {
    ...
    if (code === "INTERNAL_SERVER_ERROR") {
        return {
            outcome: "ambiguous",
            code,
            message: "Shopify returned an inconclusive server response; reconciliation is required.",
        };
    }
    return {
        outcome: "permanent-error",
...
function sanitizeShopifyMessage(message) {
    return message.replace(/\s+/g, " ").trim().slice(0, 500);
}
function isRateLimitDeferred(error) {
    return (error instanceof Error && error.name === "ShopifyRateLimitDeferredError");
}
