...
import { hasShopifyScope, statusFromGrantedScopes, } from "../shops/shop-capabilities.server.js";
export async function ingestAppLifecycleWebhook(prisma, input) {
    ...
    const grantedScopes = topic === "app/scopes_update"
    ...
    try {
        return await prisma.$transaction(async (tx) => {
            ...
            if (topic === "app/uninstalled") {
                ...
                const hasCurrentSession = (await tx.session.count({ where: { shop: input.shopDomain } })) > 0;
                const canReactivate = shop.status !== "UNINSTALLED" || hasCurrentSession;
                await tx.shop.update({
                    where: { id: shop.id },
                    data: {
                        grantedScopes,
...
export async function processAppLifecycleWebhook(prisma, input) {
    const now = input.now ?? new Date();
                ...
                if (hasShopifyScope(receipt.shop.grantedScopes, "write_orders")) {
                    ...
                    await tx.orderIntent.updateMany({
                        ...
                        data: {
                            ...
                            sanitizedLastError: "Order creation is paused until write_orders is granted.",
                            version: { increment: 1 },
                        },
                    });
...
async function resumeScopeBlockedWork(tx, shopId, now) {
    ...
    if (!hasShopifyScope((await tx.shop.findUniqueOrThrow({ where: { id: shopId } }))
...
export function extractGrantedScopes(payload) {
    ...
    const scopes = Array.isArray(current)
        ? current.filter((scope) => typeof scope === "string")
...
function normalizeTopic(topic) {
    ...
    if (normalized === "app_uninstalled")
        return "app/uninstalled";
    if (normalized === "app_scopes_update")
