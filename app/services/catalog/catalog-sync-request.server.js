...
export async function requestCatalogFullSync(prisma, input) {
...
export async function scheduleStaleCatalogSyncs(prisma, input) {
    const now = input.now ?? new Date();
    const staleCutoff = new Date(now.getTime() - input.staleAfterMinutes * 60_000);
