...
export class ShopifyRateGate {
    ...
    async reserve(shopId, estimatedCost, priority) {
        const raw = await this.redis.eval(RESERVE_BUDGET_SCRIPT, 1, this.key(shopId), this.now(), estimatedCost, this.safetyMargin, this.fallbackMaximumAvailable, this.fallbackRestoreRate, priority === "background" ? 1 : 0);
    ...
    async observe(shopId, throttleStatus) {
        await this.redis.eval(OBSERVE_BUDGET_SCRIPT, 1, this.key(shopId), throttleStatus.maximumAvailable, throttleStatus.currentlyAvailable, throttleStatus.restoreRate, this.now());
    }
    async reserveOrderCreate(shopId, reservationId) {
        const raw = await this.redis.eval(RESERVE_ORDER_CREATE_RESOURCE_SCRIPT, 2, this.orderCreateActiveKey(shopId), this.orderCreateReservationsKey(shopId), this.now(), reservationId, ORDER_CREATE_RESOURCE_LIMIT, ORDER_CREATE_RESOURCE_WINDOW_MS, ORDER_CREATE_RESOURCE_SAFETY_MS, ORDER_CREATE_RESOURCE_TTL_SECONDS);
    ...
    async activateOrderCreateLimit(shopId) {
        const raw = await this.redis.eval(ACTIVATE_ORDER_CREATE_RESOURCE_SCRIPT, 2, this.orderCreateActiveKey(shopId), this.orderCreateReservationsKey(shopId), this.now(), ORDER_CREATE_RESOURCE_LIMIT, ORDER_CREATE_RESOURCE_WINDOW_MS, ORDER_CREATE_RESOURCE_SAFETY_MS, ORDER_CREATE_RESOURCE_TTL_SECONDS);
    ...
    orderCreateActiveKey(shopId) {
        return `${this.keyPrefix}:order-create-resource-active:${shopId}`;
    }
    orderCreateReservationsKey(shopId) {
        return `${this.keyPrefix}:order-create-resource-reservations:${shopId}`;
    }
}
export function withShopifyRateGate(client, options) {
    return {
        async graphql(query, requestOptions) {
            const reservation = await options.rateGate.reserve(options.shopId, options.estimatedCost, options.priority);
            if (!reservation.allowed) {
                    ...
                    if (!bodyLoaded) {
                        ...
                        if (hasThrottleError(parsedBody)) {
                            const retryAfterMs = throttleStatus
                                ? Math.max(1_000, Math.ceil(((options.estimatedCost -
...
function hasThrottleError(body) {
    if (!isRecord(body) || !Array.isArray(body.errors))
        return false;
    return body.errors.some((error) => isRecord(error) &&
        isRecord(error.extensions) &&
        error.extensions.code === "THROTTLED");
}
export function extractShopifyThrottleStatus(body) {
    if (!isRecord(body))
        return null;
...
function parseOrderCreateReservation(value) {
