...
const nonEmpty = (maximum) => z.string().trim().min(1).max(maximum);
const optionalText = (maximum) => z
    .string()
    .trim()
    .max(maximum)
...
const rowSchema = z.object({
    ...
    unit_price: z
        .string()
        .trim()
        .regex(/^(?:0|[1-9]\d{0,9})(?:\.\d{1,2})?$/, "must be a non-negative decimal with at most two decimal places"),
...
export async function parseImportCsv(file, limits) {
    if (file.size === 0) {
    ...
    if (file.size > limits.maxBytes) {
        throw new ImportLimitError(`The file exceeds the ${formatBytes(limits.maxBytes)} upload limit.`);
    }
    let byteCount = 0;
    const byteLimiter = new Transform({
        transform(chunk, _encoding, callback) {
            ...
            if (byteCount > limits.maxBytes) {
                callback(new ImportLimitError(`The file exceeds the ${formatBytes(limits.maxBytes)} upload limit.`));
                return;
            }
            callback(null, chunk);
        },
    ...
    try {
        for await (const untrustedRecord of parser) {
            ...
            if (recordCount > limits.maxRows) {
                throw new ImportLimitError(`The CSV exceeds the ${limits.maxRows.toLocaleString("en-US")} row limit.`);
            }
            ...
            if (!parsed.success) {
                ...
                issues.push(...parsed.error.issues.slice(0, remainingIssueSlots).map((issue) => ({
                    row: rowNumber,
                    field: issue.path[0]?.toString() ?? null,
                    message: friendlyValidationMessage(issue.path[0]?.toString(), issue.message),
                })));
                continue;
            }
...
function withOptionalDefaults(record) {
    return Object.fromEntries([...REQUIRED_COLUMNS, ...OPTIONAL_COLUMNS].map((column) => [
        column,
        record[column] ?? "",
    ]));
}
...
function groupCanonicalOrders(rows) {
    ...
    for (const [externalOrderId, group] of groups) {
        ...
        for (const row of group.slice(1)) {
            if (JSON.stringify(canonicalOrderFields(row.value)) !==
...
function canonicalOrderFields(row) {
    return {
...
function friendlyValidationMessage(field, fallback) {
    switch (field) {
        case "quantity":
