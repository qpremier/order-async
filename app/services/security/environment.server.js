...
const positiveInteger = (defaultValue) => z.preprocess(emptyStringToUndefined, z.coerce.number().int().positive().default(defaultValue));
const positiveNumber = (defaultValue) => z.preprocess(emptyStringToUndefined, z.coerce.number().positive().default(defaultValue));
const decimalRatio = (defaultValue) => z.preprocess(emptyStringToUndefined, z.coerce.number().gt(0).lte(1).default(defaultValue));
const postgresUrl = requiredUrl.refine((value) => value.startsWith("postgresql://") || value.startsWith("postgres://"), "must be a PostgreSQL connection URL");
const redisUrl = requiredUrl.refine((value) => value.startsWith("redis://") || value.startsWith("rediss://"), "must be a Redis connection URL");
...
export class EnvironmentValidationError extends Error {
    issues;
    constructor(issues) {
        super(`Environment validation failed: ${issues
...
export function validateEnvironment(source = process.env) {
    ...
    if (!parsed.success) {
        throw new EnvironmentValidationError(parsed.error.issues.map(toEnvironmentIssue));
    }
    return parsed.data;
}
...
export function formatEnvironmentIssues(error) {
    if (error instanceof EnvironmentValidationError) {
        return error.issues;
    }
    return [{ name: "environment", message: "validation failed" }];
}
function toEnvironmentIssue(issue) {
    return {
